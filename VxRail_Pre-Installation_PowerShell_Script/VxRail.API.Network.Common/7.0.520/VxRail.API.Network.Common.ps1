...
function callAPI {
    param(
        ...
        [Parameter(Mandatory = $false)] $ProxyUsername,
    ...
    if($ProxyUsername){
        $ProxyCredential = getCredential -Username $ProxyUsername -Password $ProxyPassword
    }
    ...
    try{
        if($m -like "GET") {
            if ($proxy) {
                if ($ProxyCredential) {
                    $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -ContentType $ContentType -TimeoutSec 180 -Proxy $Proxy -ProxyCredential $ProxyCredential
                }
                else {
                    $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -ContentType $ContentType -TimeoutSec 180 -Proxy $Proxy
                }
            }
            else {
                $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -ContentType $ContentType -TimeoutSec 180
            }
        }
        else {
            if ($proxy) {
                if ($ProxyCredential) {
                    $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -Body $Body -ContentType $ContentType -TimeoutSec 180 -Proxy $Proxy -ProxyCredential $ProxyCredential
                }
                else {
                    $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -Body $Body -ContentType $ContentType -TimeoutSec 180 -Proxy $Proxy
                }
            }
            else {
                $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -Body $Body -ContentType $ContentType -TimeoutSec 180
            }
        }
    }catch{
