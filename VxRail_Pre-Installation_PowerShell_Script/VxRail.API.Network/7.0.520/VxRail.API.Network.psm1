...
if (-not ($HostIP -match $IPV4_PATTERN)) {
    Write-Error "Host IP is invalid, please correct it and try again"
    return
}
...
if (-not ($Gateway -match $IPV4_PATTERN)) {
    Write-Error "Gateway IP is invalid, please correct it and try again"
    return
}
    ...
    Switch($StatusCode)
        {
            '0' {
                if ($TimeDifference -le $INITIALIZE_HOST_NETWORK_SETTING_TIMEOUT) {
                    ...
                    SetAppRawData -Server $Server -Username $Username -Password $Password -HostIP $HostIP -Netmask $Netmask -Gateway $Gateway -Vlan $Vlan -StatusCode 0 -Message "init" `
                     -Proxy $Proxy -ProxyUsername $ProxyUsername -ProxyPassword $ProxyPassword
                    return
                }
            }
