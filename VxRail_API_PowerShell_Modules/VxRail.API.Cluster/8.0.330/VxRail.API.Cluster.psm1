...
<#
.Synopsis
Configure the cluster EVC mode.
.Parameter Server
VxM IP or FQDN.
.Parameter Username
Valid vCenter username which has either Administrator or HCIA role.
...
Use corresponding password for username.
.Parameter EvcMode
Required. Cluster EVC mode.
.Parameter Format
Print JSON style format.
.Example
C:\PS>Update-EvcMode -Server <vxm ip or FQDN> -Username <username> -Password <password> -EvcMode <evc_mode>
Update cluster EVC mode.
#>

function Update-EvcMode{
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
    ...
    $uri = '/rest/vxm/v1/cluster/evc'
    $url = "https://" + $Server + $uri

    try {
        $response = doPost -Server $server -Api $uri -Username $username -Password $password -Body $requestBody -ContentType "application/json"
        if ($Format) {
            $response = $response | ConvertTo-Json
        }
        return $response
