...
.Description
Export the advisor report that is generated using the provided parameters.
...
.Parameter ReportFormat
The report's format. Supported ReportFormat: 'CSV', 'XLSX'
...
.Example
Export-AdvisorReport -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -ReportFormat <Report format> -Id <Report Id> -OutputFile <Output File Path>
...
function Export-AdvisorReport {
     ...
     param(
        ...
        # The report's format
        [Parameter(Mandatory = $false, HelpMessage="Supported ReportFormat: 'CSV', 'XLSX'")]
        ...
        $uri = '/rest/vxm/v1/cvs/report/' + $Id + '/' + $ReportFormat.ToLower()      
...
<#
.Synopsis
Get history of advisor report

.Description
Getting the history record of advisor report.
...
.Example
Get-AdvisorReportHistory -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password>

#>

function Get-AdvisorReportHistory {
...
<#
.Synopsis
Retrieve available online LCM updates from Dell update server

.Description
Retrieve a list of available LCM update bundles from the Dell update server. Used to determine update_id for staging.
...
.Parameter Type
Optional. Type of bundle to retrieve. Supported types: FULL_BUNDLE (default), PRECHECK

.Parameter Format
Print JSON style format.

.Notes
Retrieve a list of available LCM update bundles from the Dell update server.

.Example
Get-LcmAvailableOnlineUpdates -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password>

#>
function Get-LcmAvailableOnlineUpdates {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
        ...
        [Parameter(Mandatory = $false)]
        [ValidateSet('FULL_BUNDLE', 'PRECHECK')]
    ...
    $collection = [System.Web.HttpUtility]::ParseQueryString([String]::Empty)
    if ($Type) { $collection.Add('type', $Type) }

    $param = $collection.ToString()
    $uri = '/rest/vxm/v1/lcm/available-online-updates'
...
<#
.Synopsis
Start asynchronous download of a specific LCM update bundle

.Description
Start asynchronous download of a specific LCM update bundle using the update_id from the previous API.
...
.Parameter UpdateId
Required. The ID of the update to be downloaded.
...
.Example
Start-LcmBundleDownload -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -UpdateId 1234

#>
function Start-LcmBundleDownload {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
    ...
    $uri = "/rest/vxm/v1/lcm/available-online-updates/$UpdateId/download"
    $url = "https://" + $Server + $uri
...
<#
.Synopsis
Retrieve download progress status of a specific LCM update bundle

.Description
Retrieves the download progress status of a specific LCM update bundle download operation initiated by the download API.
...
.Parameter RequestId
Required. The ID of the download progress request.

.Parameter Format
Print JSON style format.

.Notes
Retrieves the download progress status of a specific LCM update bundle download operation.

.Example
Get-LcmBundleDownloadProgress -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -RequestId "5ffe7062-a590-45b8-a172-8d2cf119562e"

#>
function Get-LcmBundleDownloadProgress {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
    ...
    $uri = "/rest/vxm/v1/lcm/download-progress/$RequestId"
    $url = "https://" + $Server + $uri
...
.Description
Whether uploading a package or performing an upgrade, user can get the current status through this API.
...
.Notes
Get the current status of LCM patch operation.

.Example
Get-LcmPatchStatus -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password>

#>
function Get-LcmPatchStatus {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
    ...
    $uri = "/rest/vxm/v1/lcm/patch/status"
    $url = "https://" + $Server + $uri
...
<#
.Synopsis
Upload ESXi patch bundle

.Description
Upload a pure ESXi patch bundle and perform a dry run to verify whether the patch bundle is usable.
...
.Parameter ComponentBundle
The path of the ESXi patch bundle file to be uploaded.
...
.Example
Invoke-LcmPatchUpload -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -ComponentBundle <bundle file path>

#>
function Invoke-LcmPatchUpload {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
    ...
    $uri = '/rest/vxm/v1/lcm/patch/upload'
    $url = "https://" + $Server + $uri

    try {
        $fileBytes = [System.IO.File]::ReadAllBytes($ComponentBundle);
        $fileEnc = [System.Text.Encoding]::GetEncoding('iso-8859-1').GetString($fileBytes);
        $boundary = [System.Guid]::NewGuid().ToString();
        $LF = "`r`n";
        $bodyLines = (
            "--$boundary",
            "Content-Disposition: form-data; name=`"component_bundle`"; filename=`"$(Split-Path -Leaf -Path $ComponentBundle)`"",
            "Content-Type: application/octet-stream$LF",
            $fileEnc,
            "--$boundary--$LF"
        ) -join $LF
...
<#
.Synopsis
Run precheck for ESXi patch

.Description
Dry run on host to verify whether bundle is acceptable or not. If the request body includes bundle_file_locator, the user must upload the file to the specified location themselves. The system will fetch it during the precheck execution.
...
.Parameter BundleFileLocator
Optional. The file path location of the bundle to precheck.

.Parameter Format
Print JSON style format.

.Notes
Dry run on host to verify whether bundle is acceptable or not.

.Example
Start-LcmPatchPrecheck -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -BundleFileLocator "/tmp/VMware-ESXi-8.0U3e-24674464-depot.zip"

#>
function Start-LcmPatchPrecheck {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
        ...
        [Parameter(Mandatory = $false)]
        [String] $BundleFileLocator,

        [Parameter(Mandatory = $false)]
        [Switch] $Format
    )

    $uri = "/rest/vxm/v1/lcm/patch/precheck"
    $url = "https://" + $Server + $uri

    $Body = @{}

    if ($BundleFileLocator) {
        $Body.add("bundle_file_locator", $BundleFileLocator)
    }
...
.Description
Cancel the ongoing ESXi upgrade operation.
...
.Example
Stop-LcmPatchUpgrade -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password>

#>
function Stop-LcmPatchUpgrade {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
    ...
    $uri = "/rest/vxm/v1/lcm/patch/cancel"
    $url = "https://" + $Server + $uri
...
.Description
Upgrade ESXi using the uploaded patch bundle.
...
.Parameter WitnessUsername
Witness node username.

.Parameter WitnessPassword
Witness node password.

.Parameter AutoWitnessUpgrade
Whether to automatically upgrade the witness node.
...
.Example
Start-LcmPatchUpgrade -Server <vxm ip or FQDN> -Username <VC username> -Password <VC password> -VcAdminUsername <vc admin username> -VcAdminPassword <vc admin password> -WitnessUsername <witness username> -WitnessPassword <witness password> -AutoWitnessUpgrade $true

#>
function Start-LcmPatchUpgrade {
    param(
        [Parameter(Mandatory = $true)]
        [String] $Server,
        ...
        [Parameter(Mandatory = $false)]
        [bool] $AutoWitnessUpgrade = $false,

        [Parameter(Mandatory = $false)]
        [Switch] $Format
    )

    $uri = "/rest/vxm/v1/lcm/patch/upgrade"
    $url = "https://" + $Server + $uri
