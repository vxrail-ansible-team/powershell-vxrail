...
.Parameter Version
Optional. API version. Only support v1,v2, default value is v2.
.EXAMPLE
...
function Invoke-StorageProvision {
    ...
    $uri = '/rest/vxm/' + $Version.ToLower() + '/system/primary-storage'
    $url = "https://" + $Server + $uri
...
<#
.SYNOPSIS
Update primary datastore ID to the correct ID of the datastore with the name datastore name.
.PARAMETER Server
VxM IP or FQDN.
.Parameter Username
Valid vCenter username which has either Administrator or HCIA role.
...
Required. Primary datastore name.
.EXAMPLE
PS> Update-DatastoreID -Server <VxM IP> -Username <username> -Password <password> -DatastoreName <primary storage name>
#>
function Update-DatastoreID {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        # VxManager ip address or FQDN
    ...
    $uri = '/rest/vxm/v1/system/datastore/id'
    $url = "https://" + $Server + $uri

    try{
        $response = callAPI -Username $Username -Password $Password -Server $Server -Api $uri -Method 'PATCH' -ContentType "application/json" -Body $requestBody -TimeoutSec 600
        if($Format) {
            $response = $response | ConvertTo-Json
        }
        return $response
...
<#
.SYNOPSIS
Update VxRail Manager IP address.
.PARAMETER Server
VxM IP or FQDN.
.Parameter Username
Valid vCenter username which has either Administrator or HCIA role. 
...
Print JSON style format.
.EXAMPLE  
PS> Update-VXMIPaddress -Server <VxM IP> -Username <username> -Password <password> -Conf <Json file to the path>
Update VxRail Manager IP address.
#>
function Update-VXMIPaddress {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        # VxM IP
    ...
    $uri = "/rest/vxm/" + $Version.ToLower() + "/system/vxm-ipaddress"
    $url = "https://" + $Server + $uri
    $body = Get-Content $Conf
...
Print JSON style format.
.EXAMPLE  
PS> Update-VCIPaddress -Server <VxM IP> -Username <username> -Password <password> -Conf <Json file to the path>
SUpdate vCenter IP address.
#>
function Update-VCIPaddress {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        # VxM IP
    ...
    $uri = "/rest/vxm/" + $Version.ToLower() + "/system/vcenter-ipaddress"
    $url = "https://" + $Server + $uri
    $body = Get-Content $Conf
