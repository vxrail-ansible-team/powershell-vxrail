...
.Parameter Version
Optional. API version. Only input v1, v2, or v3. Default value is v1.
...
Serial Number for the cluster. Deprecate since Version v3.
...
.Example
C:\PS>Enable-CallHome -Version v3 -Server <vxm ip or FQDN> -Username <username> -Password <password> -ApplianceID <appliance ID> -UniversalKey $False -Pin <Pin number> -AccessKey <AccessKey> -ProxyType user -ProxyProtocol HTTP -ProxyHost <Proxy IP or FQDN> -ProxyUser <proxy user> -ProxyUserPassword <proxy password> -CustomerContactInfos @(@{contact_order=1;first_name="Rob";last_name="Gordon";phone_number="+1 (312) 555-7746";email_address="rob@championshipvinyl.biz"; pref_language="en-US"}) -IPVersion IPV6
...
function Enable-CallHome {
    param(
       ...
       [string] $ProxyUserPassword,
        ...
        # Use appliance ID instead of serial number for v3
        [Parameter(Mandatory = $false)]
        [String] $ApplianceID
    )
    ...
    # For v3, use ApplianceID instead of SerialNumber
    if ($Version.ToLower() -eq "v3") {
        if (!$ApplianceID) {
            $ApplianceID = Read-Host "Please enter the ApplianceID (required for version v3)"
            }
        $Body["appliance_id"] = $ApplianceID
        } else {
        if (!$SerialNumber) {
            $SerialNumber = Read-Host "Please enter the SerialNumber (required for version v1 and v2)"
            }
        $Body["serial_number"] = $SerialNumber
        }
    
    if ($CustomerContactInfos) {
        $Body["customer_contact_infos"] = $CustomerContactInfos
        }
