...
<#
.Synopsis
Updates the ESXi hosts management user passwords stored in VxRail Manager for V3 API.
.Parameter Server
VxM IP or FQDN.
.Parameter Username
...
.Notes
This cmdlet can be used to update the ESXi hosts management user passwords stored in VxRail Manager using the V3 API.
.Example
...
This updates the ESXi hosts management user passwords stored in VxRail Manager using the V3 API.
#>
function Update-SystemCredentialV3 {
    param(
        ...
        # Component username (limited to 'ESXI')
        [Parameter(Mandatory = $true)]
        [String[]] $ComponentUsername,
