...
function callAPI {
    ...
    try{
        if($m -like "GET") {
            if($OutFile) {
                $response = Invoke-RestMethod -Method $m -Uri $url -Headers $headers -ContentType $ContentType -TimeoutSec $TimeoutSec -OutFile $OutFile
            } else {
...
function doGet {
    ...
    return callAPI -Username $Username -Password $Password -Server $Server -Api $Api -Method 'GET' -SpecialHeaders $SpecialHeaders -OutFile $OutFile
}
